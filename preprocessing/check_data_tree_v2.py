...
def print_directory_tree(directory: Path, indent: str = "", prefix: str = ""):
    ...
    if image_counts:
        # If there are image files, print the summary
        summary = " ".join(f"{count} {ext} images" for ext, count in image_counts.items())
        print(f"{indent}{'    ' if prefix else ''}└─{summary}")
    
    # Get all items in the directory (excluding image files if we already summarized them)
    items = sorted([item for item in directory.iterdir() if not (item.is_file() and item.suffix.lower() in {'.png', '.jpg', '.jpeg', '.bmp', '.gif'})],
